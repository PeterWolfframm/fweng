...
<template>
  <div @click="navigateToPost">
    <CardFoundationComponent :is-active="isActive" :variant="variant" :content-class="contentClass" :show-emoji="showEmoji">
      ...
      <div v-if="groupName || sharedBy" class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 flex flex-wrap items-center gap-2">
        <!-- Group Badge -->
        ...
        <!-- Shared By -->
        <div v-if="sharedBy" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-xs font-medium">
          <span>@{{ sharedBy }}</span>
        </div>
      </div>
